Public Class transferform
    ...
    Private Sub Button3_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button3.Click
        ...
        cmd.ExecuteNonQuery()
        cmd.Dispose()
        cmd.CommandText = "select accountamount from cust1 where accountno='" & TextBox7.Text & "'"
        cmd.Connection = vcon : rs = cmd.ExecuteReader : rs.Read()
        acont = rs(0)
        rs.Close()
        acont = acont + TextBox2.Text
        cmd = New SqlClient.SqlCommand("update cust1 set  accountamount='" & acont & "' where accountno='" & TextBox7.Text & "'", vcon)
        cmd.ExecuteNonQuery()
        cmd.Dispose()
        number1 = number1 + 1
        TextBox8.Text = Format(Now, "dddd, MMMM d,yyy hh:mm:ss tt")
        cmd = New SqlClient.SqlCommand("insert into trans values(" & number1 & ",'Transfer Money','" & TextBox8.Text & "','" & TextBox1.Text & "','" & TextBox7.Text & "','" & TextBox2.Text & "')", vcon)
        cmd.ExecuteNonQuery()
        cmd.Dispose()
        MsgBox("Amount is successfully Transfered", MsgBoxStyle.Information, "Information")
        Me.Dispose()
    End Sub
